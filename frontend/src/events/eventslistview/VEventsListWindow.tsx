...
import {CSignUpWindow, updateUSView} from "../signupview/CEventSignUpWindow";
import {SignUpState} from "../types/SignUpState";

export const VEventsListWindow:
    FC<{
        isActive: boolean;
    ...
    useEffect(
        () => {
            ...
            onLoadPageEvent("Zapytać bo nie powinno być argumentów a się wykrzacza bez XD")
        }
        , [])
                ...
                <WindowContent>
                    ...
                    >
                        <ScrollView style={{width: '450px', height: '200px'}}>
                            {elData && elData.eventsList &&
                                elData.eventsList.map(data => {
                                    return <MenuListItem onClick={() => {
                                        onClickEvent(data.uuid)
                                    }}
                                                         key={data.uuid}
                                                         style={elData && data.uuid === elData.selectedEventId ? {
                                                             background: "#05007e",
                                                             color: "white"
                                                         } : {}}
                                    >
                            ...
                            <Button onClick={() => {
                                    ...
                                    onClickCapture={() => {
                                        if (elData && elData.selectedEventId) {
                                            onSignUpEvent({eventId: elData.selectedEventId, userId: "USERID"})
                                        }
                                    }}
                                    active={!(elData && elData.eventsList.map(e => e.uuid).includes(elData.selectedEventId))}
                            >
                                Zapisz się
                            </Button>
                        </div>
